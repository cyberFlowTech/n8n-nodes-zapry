...
// Interface in Zapry
export interface IZapryInlineReply {
	inline_keyboard?: IZapryKeyboardButton[][];
}

export interface IZapryKeyboardButton {
	[key: string]: string | number | boolean;
}

export interface IZapryReplyKeyboard extends IMarkupReplyKeyboardOptions {
	keyboard: IZapryKeyboardButton[][];
}
...
export function addAdditionalFields(
	...
	if (operation === 'sendMessage') {
		...
		const link = createUtmCampaignLink('n8n-nodes-base.zapry', instanceId);
	...
	if (['inlineKeyboard', 'replyKeyboard'].includes(replyMarkupOption)) {
		...
		// @ts-ignore
		(body.reply_markup as IZapryInlineReply | IZapryReplyKeyboard)[setParameterName] =
			[] as IZapryKeyboardButton[][];
		let sendButtonData: IZapryKeyboardButton;
		if (keyboardData.rows !== undefined) {
			for (const row of keyboardData.rows) {
				const sendRows: IZapryKeyboardButton[] = [];
				...
				// @ts-ignore
				const array = (body.reply_markup as IZapryInlineReply | IZapryReplyKeyboard)[
					setParameterName
				] as IZapryKeyboardButton[][];
				array.push(sendRows);
			}
